...
def results_recorder(solver_param,state,rom_param=None):
    ...
    if solver_param['iter'] == 0:
        ...
        np.save(os.path.join(dir_results,'basis'         ,f"{save_title}_linear_basis.npy")   , rom_param['basis'])
        np.save(os.path.join(dir_results,'basis'         ,f"{save_title}_quadratic_basis.npy"), rom_param['quad_basis'])
...
def advance_one_time_step(solver_param,state,physics,time_integration,rom_param=None):
    ...
    quad_basis       = rom_param['quad_basis']
    ...
    qr                              = rom_param['hyper_precompute'] @ decen_norm_Q_bar_new_sampling

    # compute quadratic qr
    r       = len(qr)
    iu      = np.triu_indices(r)
    quad_qr = np.outer(qr, qr)[iu]

    # find the full-state
    C                             = (basis @ qr) + (quad_basis @ quad_qr)
